  ...
  context 'has instance methods to insert data into db' do
    ...
    describe '#save' do
      it 'saves the student to the db' do
        new_student.save
        expect(DB[:conn].execute("SELECT * FROM students WHERE name = 'Sam'")).to eq([{"id"=>1, "name"=>"Sam", "grade"=>11, 0=>1, 1=>"Sam", 3=>11}])
        # expect(DB[:conn].execute("SELECT * FROM students WHERE name = 'Sam'")).to eq([{"id"=>1, "name"=>"Sam"}])
      end
